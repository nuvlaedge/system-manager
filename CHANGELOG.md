...
### Changed
 ...
 - Remove internal UI and API
 - Do not fail if min requirements are not met
 - Do not consider "Docker Swarm mode enabled" a requirement, only inform if not enabled
 - Increased the interval between executions from 5 to 15 seconds. 
 - Improved logging. 
 - Added argument parser to define log level with args or env vars. 
 - Added main function. 
 - Print thread stacktaces on SIGUSR1. 
 - Bugfixes 
 - Add NUVLAEDGE_DATA_GATEWAY_ENABLED to allow to disable the data-gateway
